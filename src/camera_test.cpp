...
namespace uvc_camera_test {

CameraTest::CameraTest(ros::NodeHandle _comm_nh, ros::NodeHandle _param_nh) :
      node(_comm_nh), pnode(_param_nh), it(_comm_nh),
      info_mgr(_comm_nh, "camera"), cam(0) {
      std::cout<<"ZACH DEBUG : "<<"cameratest init"<<std::endl;
      ...
      if (pnode.getParam("auto_exposure", auto_exposure)) {
        std::cout<<"ZACH DEBUG : "<<"auto_exposure: "<<auto_exposure<<std::endl;
        int val;
        if (auto_exposure) {
          std::cout<<"EXPOSURE_AUTO"<<std::endl;
          val = V4L2_EXPOSURE_AUTO;
        } else {
          val = V4L2_EXPOSURE_MANUAL;
          std::cout<<"EXPOSURE_MANUAL"<<std::endl;
        }
      ...
      std::cout<<"exposure_absolute: "<<exposure_absolute<<std::endl;
      int exposure;
      ...
      ok = true;
      image_thread = boost::thread(boost::bind(&CameraTest::feedImages, this));
    }

    void CameraTest::sendInfo(ImagePtr &image, ros::Time time) {
    ...
    void CameraTest::sendInfoJpeg(ros::Time time) {
      CameraInfoPtr info(new CameraInfo(info_mgr.getCameraInfo()));
      info->header.stamp = time;
      info->header.frame_id = frame;
    ...
    void CameraTest::ExposureLoop(){

        unsigned char *img_frame = NULL;
        uint32_t bytes_used;
        char m_file_rgb[100];
        FILE *m_file_time = fopen("/home/zach/robot_slam_ws/src/dso/test/times.txt","w");
        int idx = cam->grab(&img_frame, bytes_used);
        cv::Mat m=cv::Mat(height,width,CV_8UC3);
        int count=0,max_count=1000;
        double exposure_time=0;
        double exposure_time_limit=20.0;
        double step=double(exposure_time_limit/max_count);
      while(1){
        double secs = ros::Time::now().toSec();
        int value = exposure_time*10;
        cam->set_v4l2_control(V4L2_CID_EXPOSURE_ABSOLUTE, value, "exposure_absolute");
        if(exposure_time>exposure_time_limit)
          break;//exposure_time=0;
        sprintf(m_file_rgb,"%s%04d%s","/home/zach/robot_slam_ws/src/dso/test/exposure_files/",count,".jpg");
        memcpy(m.data,img_frame,3*width*height*sizeof(unsigned char));
        imwrite(m_file_rgb,m);
        std::cout<<"count: "<<count<<" exposure_time: "<< exposure_time <<" value: "<<value<<std::endl;
        fprintf(m_file_time,"%05d %.4f %.4f\n",count,secs,exposure_time);
        exposure_time=exposure_time+step;
        count++;
        usleep(30000);
        //std::cout<<"exposure: "<<value<<std::endl;
      }
      fclose(m_file_time);
    }
    void CameraTest::SaveImage(int looptime){
        ...
        int idx = cam->grab(&img_frame, bytes_used);
        cv::Mat m=cv::Mat(height,width,CV_8UC3);
        int count=0,max_count=500;
        double exposure_time=3;//3ms
        //double exposure_time_limit=20.0;
        //double step=double(exposure_time_limit/max_count);
        int value = exposure_time*10;
        cam->set_v4l2_control(V4L2_CID_EXPOSURE_ABSOLUTE, value, "exposure_absolute");
